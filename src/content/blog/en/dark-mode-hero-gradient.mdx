...
description: "The Otarski homepage hero has a gradient in dark mode — brand colour at the top fading to black at the bottom. In light mode, there is no gradient at all. Here is how it works."
publishedAt: 2026-04-16
author: "Team"
tags: ["otarski", "features", "dark-mode", "css"]
featured: false
locale: en
...
Dark mode in Otarski is class-based, not media-query-based. The custom variant is defined in `global.css`:

```css
@custom-variant dark (&:where(.dark, .dark *));
```

This means the `.dark` class is toggled on the root element based on the user's explicit theme choice during the current session. The `.dark .hero-dark-gradient` rule activates the moment that class is present — no JavaScript is needed for the gradient itself. See [Why Otarski Uses sessionStorage for Dark Mode](/blog/dark-mode-sessionstorage) for how that choice is stored and restored between page loads.
...
`var(--brand-600)` is not a fixed hex value. It is a CSS custom property that updates when the user picks a different colour theme — blue, purple, orange, or any other option in the header colour picker. The gradient adapts automatically. Switch from blue to orange and the top of the hero becomes orange. No extra code needed; the custom property does the work. See [How Otarski's Design System Works](/blog/design-system-color-tokens) for a full walkthrough of every colour token and how they are structured.
